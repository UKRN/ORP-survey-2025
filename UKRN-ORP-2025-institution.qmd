...
![](logo.png){style="float:right; width: 25%"}
...
y <- ceiling((max(meth_summary$n) + 1)/5) * 5
...
y <- ceiling((max(disc_data$n) + 1)/5) * 5
...
y <- ceiling((max(career_data$n) + 1)/5) * 5
n_plot(career_data, "Career_level", title, ylim = c(0, y))
...
aware_disc |>
  ...
  geom_text(aes(label = ifelse(value == "Aware", n, "")),
            color = "white",
            y = .01,
            hjust = 0)
```
